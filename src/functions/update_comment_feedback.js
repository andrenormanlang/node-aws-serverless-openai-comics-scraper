...
async function getPersonalValuesForUser(userId) {
  if (!userId) return [];

  const newStructureParams = {
    TableName: defs.WN_STR_KEY_TABLE,
  ...
  const result = await dynamoDbLib.call("query", newStructureParams);
  const newStructureValues = (result.Items || [])
    .filter((item) => item.id && item.id.endsWith("#personalValues"))
    .map((item) => ({
      subjectId: Number(item.sortKey) || 0,
      value: Number(item.value) || 0,
    }))
    .filter((item) => item.subjectId > 0);

  if (newStructureValues.length > 0) {
  ...
  return ((legacyResult.Item && legacyResult.Item.subjects) || [])
    .map((item) => ({
